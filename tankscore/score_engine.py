...
+      - Phase 3: streak tracking + awards at end
+    """
+    def __init__(self):
...
+        self.squads = {}
+        # guard concurrency between webhook & discord thread
...
+        with self._lock:
+            # remove sector from both, then add to owner
+            self.control["ALLIES"].discard(sector_id)
+            self.control["AXIS"].discard(sector_id)
+            if new_team:
+                self.control[new_team].add(sector_id)
+            # mid assumed sector_id == 3 (adjust if your repo uses a different map abstraction)
